...
def _content_type_for_jd(file_name: str, content_type: str) -> str:
...
async def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    content_type = _content_type_for_jd(file_name, content_type)

    if extension not in ALLOWED_EXTENSIONS or content_type not in ALLOWED_CONTENT_TYPES:
        return error_response(
            error_type="validation",
            message="Only PDF and DOCX job descriptions are allowed.",
            trace_id=trace_id,
            req=req,
            status_code=400,
        )
    ...
    if len(file_content) <= 0 or len(file_content) > MAX_JD_BYTES:
        return error_response(
            error_type="validation",
            message="Job description file size exceeds allowed limit (5 MB).",
            trace_id=trace_id,
            req=req,
            status_code=400,
        )
    ...
    # Upload to blob storage with unique filename under the jds/ prefix
    blob_path = f"jds/{uuid.uuid4()}{extension}"
    ...
    try:
        ...
        return error_response(
            error_type="infra",
            message="Failed to upload job description to blob storage.",
            trace_id=trace_id,
            req=req,
            status_code=500,
            details={"reason": str(exc)},
